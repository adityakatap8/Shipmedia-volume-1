...
function ViewAndEditForm() {
  ...
  useEffect(() => {
    const fetchProjectData = async () => {
      try {
        const response = await axios.get(`https://media-shippers-backend.vercel.app/api/project-form/data/${projectId}`, {
 ...
 const constructS3Url = (subfolder, fileName) => {
  ...
  const handleDeleteProject = async () => {
    ...
    try {
      const response = await axios.delete(`https://media-shippers-backend.vercel.app/api/project-form/delete/${projectId}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  const handleSaveProjectInfo = async () => {
    ...
    try {
      const response = await axios.put(
        `https://media-shippers-backend.vercel.app/api/project-form/update/${projectId}`,
        {
          projectInfo: editableProjectInfo,
          creditsInfo: editableCreditsInfo,
          specificationsInfo: editableSpecificationsInfo,
